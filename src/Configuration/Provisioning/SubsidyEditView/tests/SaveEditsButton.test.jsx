...
jest.mock('@edx/frontend-platform/auth', () => ({
  ...
  getAuthenticatedHttpClient: jest.fn(() => ({
    ...
    patch: jest.fn(() => Promise.resolve({ uuid: 'test-uuid' })),
  })),
}));

const SaveEditsButtonWrapper = ({ value = hydratedInitialState }) => (
...
describe('Save edits button', () => {
  ...
  it('calls handleSubmit error state when clicked with no data', async () => {
    const value = { ...hydratedInitialState, formData: sampleSingleEmptyData };
  ...
  it('confirming rejected catalog update handles error via API', async () => {
    ...
    fireEvent.click(screen.getByText(SAVE_BUTTON.submit));
